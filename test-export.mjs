...
(async () => {
  ...
  try {
    ...
    await page.goto('http://localhost:3000/projects/de9709d8-ce75-45eb-90c4-76885e812f0e', {
    ...
    const exportButton = page.locator('button:has-text("Export for AI")').first();
    ...
    if (isButtonVisible) {
      ...
      const startDateButton = page.getByRole('button', { name: /^Start Date|Pick a date/ }).first();
      ...
      const createButton = page.locator('button:has-text("Start Export"), button:has-text("Create Export"), button:has-text("Export")').last();
      ...
      if (await createButton.isVisible().catch(() => false)) {
        ...
        const startedToast = page.locator('[role="status"]:has-text("Export started")').first();
        if (await startedToast.isVisible().catch(() => false)) {
          ...
          console.log('⚠️  No "Export started" toast seen (may have auto-dismissed)');
        }
        ...
        try {
          ...
          throw new Error('No Download button appeared on /profile/exports within 30s');
        }
        ...
        if (s3Errors.length > 0) {
          ...
          const okToast = page.locator('[role="status"]:has-text("Download started")').first();
